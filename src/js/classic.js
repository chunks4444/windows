    ...
    function startAISynthesis() {
        ...
        .catch(e => { overlay.style.display = 'none'; pmAlert(_t('렌더링 중 오류: ') + e.message, { type: 'danger' }); });
    }
...
async function draw() {
    ...
    async function restoreThumbs() {
        ...
        try {
            ...
            const wallpaperPromises = data.wallpapers.map(({ id: serverId, filename, url: src }) => {
                return new Promise(resolve => {
                    ...
                    imgObj.onload = function() {
                        if (mySeq !== _restoreThumbsSeq) { resolve(); return; } // 그 사이 다른 도면으로 전환됨 — 폐기
            ...
            Promise.all(wallpaperPromises).then(() => { if (mySeq === _restoreThumbsSeq) draw(); });
        } catch(e) {}
    }
        ...
        drawings.forEach(d => {
            ...
            item.innerHTML = `
                <div class="dm-item-info">
                    <div class="dm-title">${escHtml(d.title)}${d.locked_at ? ` <i class="bi bi-lock-fill dm-lock-icon" title="${escHtml((ORDER_STATUS_LABELS[d.order_status] || {}).label || _t('견적요청 중'))}"></i>` : ''}${d.pattern_category ? ` <span class="dm-category-badge">${escHtml(d.pattern_category_name || d.pattern_category)}</span>` : ''}</div>
