    ...
    let _pmModalEl = null;
    // ── 렌더링 결과 저장 ───────────────────────────
    const RENDERS_KEY = 'pmok_hexagon_renders';
    const MAX_RENDERS = 9;
    let savedRenders = [];
...
async function draw() {
    ...
    for (const d of renderOrder) {
        ...
        // 클리핑 설정
        if (buildKonvaPattern) {
            kv.addPatternClipGroup(d, iLeft, iTop, iW, iH);
        } else {
            ctx.save();
            ctx.beginPath();
            ...
            for (let y = startY - rowStep, rIdx = 0; y < iTop + iH + rowStep; y += rowStep, rIdx++) {
                for (let x = iLeft - width, cIdx = 0; x < iLeft + iW + width; x += width, cIdx++) {
                    ...
                    for (let i = 0; i < 6; i++) {
                        ...
                        const x1 = cx + size * Math.cos(a1);
                        const y1 = cy + size * Math.sin(a1);
                        const x2 = cx + size * Math.cos(a2);
                        const y2 = cy + size * Math.sin(a2);
                        const mx = (x1 + x2) / 2;
                        const my = (y1 + y2) / 2;
                        const edgeAngle = Math.atan2(y2 - y1, x2 - x1);
                        const normAngle = ((edgeAngle % Math.PI) + Math.PI) % Math.PI;
            ...
            if (faceColorMap) {
                for (let i = 0; i < geo.cols; i++) {
                    ...
                    for (let k = -1; ; k++) {
                        ...
                        for (let w = 0; w < 6; w++) {
                            const _fc = faceColorMap[`hex:${i}:${k}:${w}`] ?? null;
                            if (_fc) {
    ...
    function hitTestCell(px, py) {
        ...
        const iLeft = lastILeft, iTop = lastITop, iW = lastIW;
...
document.getElementById('muntolColorInput')?.addEventListener('input', e => { selectedMuntolColor = e.target.value; document.getElementById('muntolColorCode').textContent = e.target.value; draw(); });
    ...
    // 작성일 / 수정일
    const CREATED_KEY       = 'pmok_hexagon_created';
    const MODIFIED_KEY      = 'pmok_hexagon_modified';
    const VERSIONS_KEY      = 'pmok_hexagon_versions';
    const BG_IMAGE_KEY      = 'pmok_hexagon_bg';
    const WALLPAPER_ENGINE  = 'hexagon';
    const CURRENT_TITLE_KEY = 'pmok_hexagon_current_title';
    const NAME_KEY          = 'pmok_hexagon_name';
    ...
    async function tryResumePendingSave() {
        ...
        if (pending.type !== 'hexagon') return false;
    ...
    async function loadFromDb(title) {
        const data = await /** @type {any} */ (window.DrawingSync).load('hexagon', title);
        ...
        drawings.forEach(d => {
            ...
            item.querySelector('.dm-del-btn').addEventListener('click', (e) => {
                ...
                pmConfirm(`"${d.title}" 도면을 삭제하시겠습니까?`, async () => {
                    await /** @type {any} */ (window.DrawingSync).delete('hexagon', d.title);
    ...
    document.getElementById('dmRenameOk').addEventListener('click', async () => {
        ...
        const ok = await /** @type {any} */ (window.DrawingSync).rename('hexagon', _renameTarget, newTitle);
        if (ok) {
