            ...
            for (let i = 1; i < geo.cols; i++) {
                ...
                for (let j = 1; j < geo.rowsInt; j++) {
                    ...
                    for (let i = 0; i < geo.cols; i++) {
                        ...
                        if (buildKonvaPattern) {
                            kv.addPatternSlatRect(d, toCanvasX(segStartX), toCanvasY(top), (segEndX - segStartX) * baseScale, geo.slatH * baseScale, Color_Slat_Fill, hsKey, lastSegMap.get(hsKey).lineKey);
                        } else {
                            ctx.fillStyle = Color_Slat_Fill;
...
document.getElementById('muntolColorInput')?.addEventListener('input', e => { selectedMuntolColor = e.target.value; document.getElementById('muntolColorCode').textContent = e.target.value; draw(); });
    ...
    async function tryResumePendingSave() {
        ...
        if (pending.type !== 'square') return false;
            ...
            if (dup) {
                pmConfirm(
                    ...
                    async () => {
                        saveBtn.classList.add('save-busy'); saveBtn.disabled = true; pmShowSaveToast(_t('저장 중...'), true);
                        try {
                            const loaded = await /** @type {any} */ (window.DrawingSync).load('square', title);
