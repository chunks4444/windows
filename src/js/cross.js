    ...
    // 다시 찾을 때 씀 (cross 패턴은 대각선이 셀 중심에서도 만나서 교점이 두 종류)
    ...
    let _pmModalEl = null;
    // ── 렌더링 결과 저장 ───────────────────────────
    const RENDERS_KEY = 'pmok_cross_renders';
    const MAX_RENDERS = 9;
    let savedRenders = [];
...
async function draw() {
    ...
    for (const d of renderOrder) {
...

        {
            ...
            // ====================================
            const _diagStroke = patternBroken ? '#cc0000' : selectedSlatColor;
            const _diagW = geo.slatT * baseScale;
            if (!buildKonvaPattern) {
                ctx.strokeStyle = _diagStroke;
                ctx.lineWidth = _diagW;
                ctx.lineCap = 'round';
            }
...
document.getElementById('muntolColorInput')?.addEventListener('input', e => { selectedMuntolColor = e.target.value; document.getElementById('muntolColorCode').textContent = e.target.value; draw(); });
    ...
    // 작성일 / 수정일
    const CREATED_KEY       = 'pmok_cross_created';
    ...
    async function tryResumePendingSave() {
        ...
        if (pending.type !== 'cross') return false;
    ...
    function showVerDelConfirm(label, onConfirm) {
        pmConfirm(_t('%s를 정말 삭제하시겠습니까?', label), onConfirm, { type: 'danger' });
    }
    ...
    async function loadFromDb(title) {
        const data = await /** @type {any} */ (window.DrawingSync).load('cross', title);
    ...
    async function openDrawingByTitle(title) {
        const data = await /** @type {any} */ (window.DrawingSync).load('cross', title);
        if (!data || !data.versions || !data.versions.length) { pmAlert(_t('도면을 불러올 수 없습니다.')); return; }
    ...
    document.getElementById('dmRenameOk').addEventListener('click', async () => {
        ...
        const ok = await /** @type {any} */ (window.DrawingSync).rename('cross', _renameTarget, newTitle);
        if (ok) {
