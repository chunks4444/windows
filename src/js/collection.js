...
function buildCard(p) {
    ...
    // API는 /en/ 접두사 없는 엔진 경로를 주므로, 영문 페이지에서 한글 에디터로 떨어지지 않게 _lh()를 태운다
    const editorUrl = p.editor_url ? _lh(p.editor_url) : '';
    const editorBtn = editorUrl && p.drawing_id
        ? `<a href="${esc(editorUrl)}?drawing_id=${p.drawing_id}" class="lib-btn lib-btn-primary" onclick="return openCollectionEditor(event,'${esc(editorUrl)}?drawing_id=${p.drawing_id}')"><i class="bi bi-pencil"></i> ${_t('열기')}</a>`
        : '';
