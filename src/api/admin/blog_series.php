...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $rows = $pdo->query("SELECT id, name, tagline, sort_order FROM blog_series ORDER BY sort_order, id")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pdo->prepare("INSERT INTO blog_series (name, tagline, sort_order) VALUES (?,?,?)")
        ->execute([$name, trim($body['tagline'] ?? ''), (int)($body['sort_order'] ?? 0)]);
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    $pdo->prepare("UPDATE blog_series SET name=?, tagline=?, sort_order=? WHERE id=?")
        ->execute([$name, trim($body['tagline'] ?? ''), (int)($body['sort_order'] ?? 0), $id]);
