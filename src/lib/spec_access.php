...
// "제작 시방서"는 2026-08-08부터 로그인/승인 여부와 무관하게 항상 공개.
// role(s/m)은 자동으로 부재목록·예상가격·납기·배송비·설명을 통과하고, role s만 예산견적 상세를 자동 통과한다.
// 그 외 회원은 users.view_parts/view_cost/view_price/view_leadtime/view_shipping/view_desc 플래그로 개별 승인.
require_once __DIR__ . '/jwt.php';
require_once __DIR__ . '/db.php';

function get_content_permissions(): array {
    $payload = jwt_from_request();
    if (!$payload) return ['spec' => true, 'parts' => false, 'cost' => false, 'price' => false, 'leadtime' => false, 'shipping' => false, 'desc' => false];
    ...
    $stmt = db()->prepare('SELECT view_parts, view_cost, view_price, view_leadtime, view_shipping, view_desc FROM users WHERE id = ?');
