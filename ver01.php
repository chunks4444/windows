...
.sb-inner { min-width: var(--sidebar-w); }
...
.spec-card.accent .spec-val { color: var(--teal); }
.spec-card.accent-blue .spec-val { color: #236B62; }
...
.sb-footer strong { color: var(--teal); font-weight: 600; }
</style>
</head>
<body>
...
<header>
    <div class="hdr-brand">
        ...
        <div class="brand-text">
            <div class="brand-title">창호 설계기</div>
            <div class="brand-sub">사분턱 정밀 도면</div>
        </div>
    </div>
    ...
    <div class="hdr-actions">
        <button class="hbtn" id="btnToggleSidebar">
        ...
        <button class="hbtn hbtn-primary" id="btnResetZoom">
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                <path d="M1 6.5C1 3.46 3.46 1 6.5 1s5.5 2.46 5.5 5.5" stroke="white" stroke-width="1.5" stroke-linecap="round"/>
                <path d="M1 6.5V3.5M1 6.5H4" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            초기화
        </button>
    </div>
...
function calculateGeometry() {
    ...
    geo = { cellSize, outerW, outerH, frameT, slatT, step, cols, rows, innerW, innerH, diagEye, tenonDepth };
...
function draw() {
    ...
    for (let i = 1; i < geo.cols; i++) {
        ...
        ctx.fillStyle = Color_Tenon_Fill; ctx.strokeStyle = '#555555';
        ...
        ctx.fillStyle = Color_Slat_Fill; ctx.strokeStyle = Color_Slat_Stroke;
...
function drawDiagonalWithTenon(body_pts) {
    ...
    // 시작 촉
    ctx.moveTo(
        toCanvasX(tsx + px * tenonHalf),
        toCanvasY(tsy + py * tenonHalf)
    );

    ctx.lineTo(
        toCanvasX(sx + px * bodyHalf),
        toCanvasY(sy + py * bodyHalf)
    );

    // 몸체 상단
    ctx.lineTo(
        toCanvasX(ex + px * bodyHalf),
        toCanvasY(ey + py * bodyHalf)
    );

    // 끝 촉
    ctx.lineTo(
        toCanvasX(tex + px * tenonHalf),
        toCanvasY(tey + py * tenonHalf)
    );
...
function clipLineToRect(x1, y1, x2, y2, rx1, ry1, rx2, ry2) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        else if (p[i] > 0) { if (r < t0) return null; if (r < t1) t1 = r; }
    }
...
window.addEventListener('mousemove', function(e) {
    if (!isDragging) return; panX = e.clientX - startX; panY = e.clientY - startY; draw();
});
...
btnResetZoom.addEventListener('click', function() {
    scaleFactor = 1.0; panX = 0; panY = 0; draw();
});
[txtW, txtCols, txtH, txtFrame, txtSlat].forEach(input => input.addEventListener('input', draw));
